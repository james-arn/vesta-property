...
import { getHighConfidenceTooltipText, getMediumConfidenceTooltipUpdate } from "./propertyAddressHelpers";
...
const AddressDetailsDisplay: React.FC<AddressDetailsDisplayProps> = ({
    ...
    if (isHighConfidence) {
        ...
        if (confidence === ConfidenceLevels.MEDIUM) {
            confidenceTooltipText = "Address confidence is medium. Please review for accuracy.";
            confidenceTooltipText += getMediumConfidenceTooltipUpdate(epcSource);
        } else { // Handles ConfidenceLevels.NONE and any other non-high, non-medium confidence levels
            confidenceTooltipText = "Address confidence is undetermined or low. Please specify or verify.";
        }
    }
